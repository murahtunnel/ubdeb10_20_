...
echo -e "\e[92;1m Update & upgrade \e[0m"
apt update

clear

echo -e "\e[92;1m install curl \e[0m"
apt install curl -y

clear

echo -e "\e[92;1m install wget \e[0m"
apt install wget -y

clear

echo -e "\e[92;1m install jq \e[0m"
apt install jq -y

clear

# Konfigurasi GitHub
GITHUB_USERNAME="murahtunnel"
GITHUB_TOKEN="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"
REPO="vps_access"
BRANCH="main"
FILE_NAME="ip"  # File daftar IP

# dapatkan ip vps
IP=$(curl -sS ipv4.icanhazip.com)

# banner and input key
clear
echo -e "\033[91;1m ================================   \033[0m"
echo -e "\033[96;1m   add vps ip to github via key     \033[0m"
echo -e "\033[91;1m ================================   \033[0m"
echo ""
read -p "INPUT Key: " KEY


# Ambil angka dari key ( menentukan masa aktif )
DAYS=$(echo "$KEY" | grep -o '[0-9]\+')
if [ -z "$DAYS" ]; then
    echo "Key tidak valid. Pastikan ada angka di dalamnya!"
    exit 1
fi
...
# Cek apakah IP sudah terdaftar
if grep -q "$IP" "$FILE_NAME"; then
    echo -e "⚠️\e[91;1m  IP $IP sudah terdaftar!"
    exit 1
fi

git config user.email "bot@murahtunnel.com"
git config user.name "Murahtunnel Bot"

ENTRY="### $NAME $EXPIRY_DATE $IP"
echo "$ENTRY" >> "$FILE_NAME"
#echo "✅ Entri ditambahkan: $ENTRY"

git add "$FILE_NAME"
git commit -m "Menambahkan IP: $IP | Masa aktif: $DAYS hari"
git push https://$GITHUB_USERNAME:$GITHUB_TOKEN@github.com/$GITHUB_USERNAME/$REPO.git $BRANCH

clear
echo -e ""
echo -e "\033[91;1m ================================   \033[0m"
echo -e "\033[93;1m   SUKSES DI TAMBAHKAN VIA KEY      \033[0m"
echo -e "\033[91;1m ================================   \033[0m"
echo -e "\033[96;1m  IP VPS   :\033[92;1m ${IP}        \033[0m"
echo -e "\033[96;1m  EXPIRED  :\033[92;1m $EXPIRY_DATE \033[0m"
echo -e "\033[91;1m ================================   \033[0m"
echo""
echo -e "\033[32m wait in 5 seccond...... \033[0m"
sleep 5
...
function Installasi(){
    animation_loading() {
        ...
        (
            # Hapus file penanda jika ada
            [[ -e $HOME/fim ]] && rm -f $HOME/fim

            # Jalankan perintah secara background dengan output disembunyikan
            ${CMD[0]} -y >/dev/null 2>&1
            ${CMD[1]} -y >/dev/null 2>&1

            # Buat file penanda untuk menandakan proses telah selesai
            touch $HOME/fim
        ) >/dev/null 2>&1 &
        ...
        echo -ne "  \033[92;1mProcessed \033[1;37m- \033[96;1m["

        # Loop animasi spinner
        while true; do
            # Jika file penanda ada, berarti proses selesai
            if [[ -e $HOME/fim ]]; then
                rm -f $HOME/fim
                break
            fi

            # Tampilkan spinner dengan mengganti karakter setiap 0.1 detik
            printf "\r  \033[92;1mProcessed \033[1;37m- \033[96;1m[ %s ]" "${spinner[idx]}"
            sleep 0.1
            idx=$(( (idx + 1) % ${#spinner[@]} ))
        done

        # Tampilkan pesan sukses setelah proses selesai
        echo -e "\r  \033[92;1mProcessed \033[1;37m- \033[96;1m[ \033[0;32mOK!\033[96;1m ]\033[1;37m"
